...
export default function Favoritar(props) {
  ...
  const verificaExistencia = () =>
    listaFavoritos.some((v) => v.id === props.id);
  ...
  const iconeFavoritar = (
    <img src={C3POSemCor} className={styles.iconeFavoritar} />
  );
  const iconeFavoritado = (
    <img src={C3POColorido} className={styles.iconeFavoritar} />
  );
  ...
  return (
    <section id={styles.sectionFavorito}>
      <Button
        id="btnFavoritar"
        onClick={aoClicar}
        style={estadoFavorito ? botaoFavoritado : botaoFavoritar}
      >
        <label className={styles.botaoLabel}>
          {estadoFavorito ? 'Favorito!' : 'Favoritar'}
        </label>
